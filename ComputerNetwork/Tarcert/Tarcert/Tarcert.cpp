...
void main()
{
	...
	for(int i=1;i<=255;i++){
		...
		char IpTemp[255];
		itoa(i,temp,10);
		strcpy(IpTemp,IpAddress);
		strcat(IpTemp,temp);
		//cout<<IpTemp<<endl;
		u_long ulDestIP=inet_addr(IpTemp);
		//转换不成功时按域名解析
		if(ulDestIP==INADDR_NONE)
		{
			hostent * pHostent=gethostbyname(IpTemp);
			if(pHostent)
			{
		...
		const DWORD DEF_ICMP_TIMEOUT=30;    //回显应答超时时间
				...
				if(iReadDataLen!=SOCKET_ERROR)//有数据到达
				{
					...
					if(DecodeIcmpResponse(IcmpRecvBuf,iReadDataLen,DecodeResult,ICMP_ECHO_REPLY,ICMP_TIMEOUT))
					{
						...
						if(DecodeResult.dwIPaddr.s_addr==destSockAddr.sin_addr.s_addr)
							bReachDestHost=true;
						//输出IP地址
						string tip = GetMAC(IpTemp);
						cout<<'\t'<<inet_ntoa(DecodeResult.dwIPaddr)<<"\t"+tip<<endl;
						break;
					}
				}
